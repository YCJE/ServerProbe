...
以下 10 条原则是不可妥协的设计红线，所有后续设计必须符合。
...
| S2 | 强制 TLS + 强制服务器身份验证 | 所有通信（Agent↔Server、浏览器↔Server）强制加密，不允许关闭。Agent 拒绝连接未启用 TLS 的 Server。**加密不等于认证**：自签名证书场景通过证书指纹固化（TOFU）验证 Server 身份，禁止提供跳过证书验证的开关 |
...
| S9 | 下发参数有界 | Server 能下发的任何参数（探测间隔/上报间隔/目标数）必须落在 Agent 硬编码的安全区间内，**上下界均不可配置**。防止 Server 沦陷后将 Agent 武器化为分布式探测流量源 |
| S10 | 永不带内更新 | Agent 二进制永不来自 Server 通道。更新仅经 GitHub Releases + 签名校验（带外），或管理员手动更新。更新器是独立脚本，绝不是协议中的一帧 |

> **残余能力底线**：假设 Server 被完全攻破，攻击者对 Agent 的影响被收敛为：读取监控数据（不可避免）、增删改有界探测目标、调整有界间隔、篡改面板数据、冒用单台 Agent 身份。**不存在**任何控制 Agent 行为、读取 Agent 主机文件、下发代码的通道。完整清单与功能准入门槛见 `SECURITY.md`。

#### 2.1.1 功能准入门槛（Non-Goals 机制）

所有新功能（尤其竞品功能借鉴）上线前必须回答三个问题：

1. **通道问题**：是否新增 Server→Agent 消息类型或参数？参数是否满足 S9 有界约束？
2. **残余能力问题**：Server 沦陷时该功能最坏能放大成什么？是否超出残余能力清单边界？
3. **执行问题**：是否需要 Agent 执行命令、写文件、建终端、发起任意连接？若是直接拒绝（S4）。

**永久排除（Non-Goals）**：Web 终端、文件管理、计划任务推送、进程管理、Agent 带内自更新、DDNS/内网穿透、多租户。详见 `SECURITY.md` 的"永不实现清单"。

> **竞品借鉴依据**：对 Komari 与 NodeGet 的源码级深度分析（功能/数据库/协议/安全）见 `COMPETITOR_ANALYSIS.md`，其中所有借鉴建议均已按本节功能准入门槛过滤分级。
...
| T13 | 中间人冒充 Server（自签名场景跳过证书验证） | S2：证书指纹固化（TOFU），Agent 校验 Server 证书 SHA-256 指纹，禁止跳过验证开关 |
| T14 | Server 沦陷后下发极端参数，将 Agent 武器化为分布式探测源 | S9：下发参数双向钳制（间隔下限 3s、目标数 ≤20），Agent 硬编码不可配置 |
| T15 | 被攻破的 Server 向 Agent 推送恶意二进制（Nezha 致命链） | S10：永不带内更新，Agent 二进制只经带外（GitHub Releases + 签名校验或手动） |
...
- **Agent 升级（S10：永不带内更新）**：
  - Agent 永不通过 Server 通道获取二进制或更新指令
  - 提供独立的带外更新脚本：从 GitHub Releases 下载 → 内嵌公钥验签 → 替换二进制 → 重启服务，由管理员在 Agent 主机上手动执行
  - 手动升级路径永久保留（下载、校验、替换）
